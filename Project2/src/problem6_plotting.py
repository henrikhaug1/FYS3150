...
for i in range(len(numerical_eigvecs)):
	numerical_eigvecs[i][2] = -1 * numerical_eigvecs[i][2]
...
for i in range(3):
	...
	ax1[i].plot(x_hat_numerical, numerical_eigvecs[:, i], label=f"Numerical Eigenvector {i+1}",linestyle="--", marker="x", color='blue')
	ax1[i].legend()
	ax1[i].grid(True)
plt.tight_layout()
plt.show()